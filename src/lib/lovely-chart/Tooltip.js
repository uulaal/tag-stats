...
export function createTooltip(container, data, plotSize, colors, onZoom, onFocus) {
    ...
    function _setupBalloon() {
        ...
        _balloon.innerHTML =
            '<div class="lovely-chart--tooltip-title"></div><div class="lovely-chart--tooltip-legend"></div><div class="lovely-chart--spinner"></div>';
    ...
    function _getBalloonLeftOffset(labelIndex) {
        ...
        return shouldPlaceRight ? _offsetX + BALLOON_OFFSET : _offsetX - (_balloon.offsetWidth + BALLOON_OFFSET);
    }
    ...
    function _insertNewDataSet(dataSetContainer, { name, key, value }, totalValue) {
        const className = `lovely-chart--tooltip-dataset-value lovely-chart--position-right lovely-chart--color-${data.colors[
            key
        ].slice(1)}`;
        ...
        newDataSet.innerHTML = `<span class="lovely-chart--dataset-title">${name}</span><span class="${className}">${formatInteger(
            value
        )}</span>`;
    ...
    function _updateDataSet(currentDataSet, { key, value } = {}, totalValue) {
        ...
        const valueElement = currentDataSet.querySelector(
            `.lovely-chart--tooltip-dataset-value.lovely-chart--color-${data.colors[key].slice(
                1
    ...
    function _renderPercentageValue(dataSet, value, totalValue) {
        ...
        const percentageElement = dataSet.querySelector(
            `.lovely-chart--percentage-title:not(.lovely-chart--state-hidden)`
        );
    ...
    function _updateDataSets(statistics) {
        ...
        const pointerVector = getPointerVector();
        const finalStatistics = data.isPie
            ? statistics.filter(({ value }, index) =>
                  _isPieSectorSelected(statistics, value, totalValue, index, pointerVector)
              )
            : statistics;
