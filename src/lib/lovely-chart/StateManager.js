...
export function createStateManager(data, viewportSize, callback) {
    ...
    function _buildTransitionConfig() {
        ...
        mergeArrays([ANIMATE_PROPS, datasetVisibilities]).forEach((transition) => {
...
function calculateYRangesForGroup(data, labelFromIndex, labelToIndex, prevState, datasets) {
    const { min: yMinMinimapReal = prevState.yMinMinimap, max: yMaxMinimap = prevState.yMaxMinimap } = getMaxMin(
        mergeArrays(datasets.map(({ yMax, yMin }) => [yMax, yMin]))
    );
