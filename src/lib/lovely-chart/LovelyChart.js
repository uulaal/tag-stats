...
function create(container, originalData) {
    ...
    function _setupComponents() {
        ...
        _zoomer =
            _data.isZoomable &&
            createZoomer(_data, originalData, _colors, _stateManager, _element, _header, _minimap, _tooltip, _tools);
    ...
    function _setupContainer() {
        ...
        _element.className = `lovely-chart--container${
            _data.shouldZoomToPie ? ' lovely-chart--container-type-pie' : ''
        }`;
    ...
    function _getCaption(state) {
        ...
        return isDataRange(_data.xLabels[startIndex], _data.xLabels[endIndex])
            ? `${getLabelDate(_data.xLabels[startIndex])}` + ' — ' + `${getLabelDate(_data.xLabels[endIndex])}`
